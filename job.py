...
class SetupCancelled(Exception):
    """Raised when user exits setup with shortcut."""
...
def _ask(label, **kwargs):
    """Prompt wrapper for setup shortcuts and cleaned input."""
    val = Prompt.ask(label, **kwargs)
    val = val.strip().strip('"').strip("'")
    low = val.lower()
    if val == BACK or low == "b":
        return None
    if low == "q":
...
def cmd_setup():
    ...
    def step_gmail():
        ...
        while True:
            ...
            if pwd == BACK or pwd.lower() == "b": return "back"
            if pwd.lower() == "q": raise SetupCancelled()
    ...
    def step_cv():
        ...
        while True:
            ...
            method = _ask("  Metodo", default="1")
            if method is None: return "back"

            cv = ""
            if method == "1":
                cv = _choose_pdf_file_dialog(cfg.get("cv_path", "")) or ""
                if not cv:
                ...
                pick = _ask("  Selecciona numero", default="1")
                if pick is None:
                    continue
                try:
                    idx = int(pick) - 1
